...
class Qualidade:
    def __init__(self):
        self.automato = Automato("projeto_py/mecanica/automatos/qualidade.txt")
        self.shut = False
    ...
    def mainthread(self,blop,obj_list,ddl):
        ...
        while not self.shut:
            ...
            if final == "chegar_cmp":
                obj_list[0].add_fita("cmp_alinhado_quald")
            ...
            if final == "sign_torno":
                obj_list[0].add_fita("torno_load")

            

            time.sleep(ddl)
        print("qualidade shut down")
